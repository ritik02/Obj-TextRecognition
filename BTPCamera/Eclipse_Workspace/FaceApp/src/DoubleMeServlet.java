       ...
       /*  
        ...
        //   url[pic+2]=new URL("http://i350.photobucket.com/albums/q410/ritikxman/nfd_zps3ou92bsp.png");
           ...
           */
                    ...
                    for(int i=0;i<pic;i++)
     {
    	 image[i] = ImageIO.read(this.getClass().getResource("/t"+(i+1)+".jpg"));
    	 image[i]=resize(image[i],nopp,nopp);
    	 image[i]=greyscales(image[i]);
     }
                   
  /*   imagetest = ImageIO.read(test);
        	       ...
        	       // Log.i("evectors", "" + ans);
        	        ...
        	        if (ee < max1) {
        	            ...
        	            if (min < max) {
        	                ...
        	                return ("PCA =" + ss[ans] + " " + (int)min);
        	               
        	            } else {
        	               
        	               return ("PCA= none " + (int)min+" but can be "+ss[ans]);
        	                //  ImageView.setImageBitmap(bitmap[pic + 1]);
        	            }
        	        } else {
        	            
        	            return ("PCA= Not Even a Face! " + (int)ee+"  but can be "+ss[ans]);
    ...
    double cmin=1000000000;int cmini=0;
         ...
         //   String ss[] = {"chacha","somu","pengu","sachin","chandak","arpit","ritik"};
            ...
            if(rmin<8000&&cmin<8000)
              return("Ext2D PCA=  "+ss[rmin<=cmin?rmini:cmini]+" "+(int)rmin);
            else
               return("Ext2D PCA= none but can be "+ss[rmin<=cmin?rmini:cmini]+" "+(int)rmin);
        //  int  num = 0;
           // Bitmap compare = Bitmap.createBitmap(nopp, nopp, Bitmap.Config.ARGB_8888);
